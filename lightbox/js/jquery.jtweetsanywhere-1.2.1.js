...
(function($)
{
	...
	defaultTweetTimestampFormatter = function(timeStamp)
	{
		...
		if (diff < 60)
		{
			tweetTimestamp += diff + ' segundos' + (diff == 1 ? '' : 's') + ' antes';
		}
		else if (diff < 3600)
		{
			var t = parseInt((diff + 30) / 60);
			tweetTimestamp += t + ' minutos' + (t == 1 ? '' : 's') + ' antes';
		}
		else if (diff < 86400)
		{
			var t = parseInt((diff + 1800) / 3600);
			tweetTimestamp += t + ' horas' + (t == 1 ? '' : 's') + ' antes';
		}
		else
		{
			var d = new Date(timeStamp);
			...
			tweetTimestamp += ' (' + t + ' día' + (t == 1 ? '' : 's') + ' antes)';
		}
	...
	exTimestampFormatter = function(timeStamp)
	{
		...
		if (diff < 60)
		{
			tweetTimestamp += 'hace menos de un minuto';
		}
		else if (diff < 3600)
		{
			var t = parseInt((diff + 30) / 60);
		...
		{
			var t = parseInt((diff + 1800) / 3600);
			tweetTimestamp += 'sobre ' + t + ' una hora' + (t == 1 ? '' : 's') + ' antes';
		}
		else
		{
			var t = parseInt((diff + 43200) / 86400);
			tweetTimestamp += 'sobre ' + t + ' un día' + (t == 1 ? '' : 's') + ' antes';
	...
	defaultLoadingDecorator = function(options)
	{
		...
		return '<li class="jta-loading">recuperando útimo tweet ...</li>';
	};
